...
void Move(char Field[], const int n)
{
	...
	else counter--;				//Если игрок выбрал НЕ пустую ячейку, он ходит ещё раз пока не выберет пустую клетку
...
void Check(char Field[], const int n)
{
	for (int i = 0; i < n; i++)
	{
		for (int c = 1; c < 5; c++)
		{
			if (Field[i] == 'X' && Field[i+1*c] == 'X' && Field[i+2*c] == 'X' && i != 4 && i != 5 && i != 7 && i != 8 && i+2*c != 3 && i+2*c != 4 && i+1*c != 2 && i+1*c != 6 && (i+2*c)-i != 4)	
			...
			if (Field[i] == '0' && Field[i+1*c] == '0' && Field[i+2*c] == '0' && i != 4 && i != 5 && i != 7 && i != 8 && i+2*c != 3 && i+2*c != 4 && i+1*c != 2 && i+1*c != 6 && (i+2*c)-i != 4)
